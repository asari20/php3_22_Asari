...
// 3.DBからデータ取得用のSQLを作成
$stmt = $pdo->prepare("SELECT * FROM gs_bm_table WHERE unique_key = :key");
        ...
        <form method="POST" action="update.php"  class="lg:w-1/2 md:w-2/3 mx-auto">
            <div class="flex flex-wrap -m-2">
                <div class="p-2 w-full">
                    ...
                    <input type="text" id="name" name="name" class="w-full bg-gray-100 bg-opacity-50 rounded border border-gray-300 focus:border-indigo-500 focus:bg-white focus:ring-2 focus:ring-indigo-200 text-base text-gray-700 py-1 px-3" require value="<?=$result['name']?>">
                    
                </div>
                <div class="p-2 w-full">
            
                    <label for="author" class="leading-7 text-sm text-gray-600">著者</label>
                    <input type="text" id="author" name="author" class="w-full bg-gray-100 bg-opacity-50 rounded border border-gray-300 focus:border-indigo-500 focus:bg-white focus:ring-2 focus:ring-indigo-200 text-base text-gray-700 py-1 px-3" require value="<?=$result['author']?>">
                ...
                <div class="p-2 w-full">
                    ...
                    <input type="text" id="url" name="url" class="w-full bg-gray-100 bg-opacity-50 rounded border border-gray-300 focus:border-indigo-500 focus:bg-white focus:ring-2 focus:ring-indigo-200 text-base text-gray-700 py-1 px-3" value="<?=$result['url']?>">
            
                </div>
                <div class="p-2 w-full">
               
                    <label for="message" class="leading-7 text-sm text-gray-600">コメント</label>
                    <input type="text" id="comment" name="comment" class="w-full bg-gray-100 bg-opacity-50 rounded border border-gray-300 focus:border-indigo-500 focus:bg-white focus:ring-2 focus:ring-indigo-200 h-32 text-base text-gray-700 py-1 px-3 resize-none" value="<?=$result['comment']?>"></input>
                </div>
            </div>
